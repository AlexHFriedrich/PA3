...
class LloydsAlgorithm:
    def __init__(self, k, data, true_labels, max_iter, random_init=False, tol=2*1e-2):
        self.tol = tol
    ...
    def _compute_loss(self):
        ...
        for i in range(self.k):
            if len(self.clusters[i]) > 0 and len(self.centroids[i]) > 0:
                loss += np.sum([np.linalg.norm(self.data[self.clusters[i]] - self.centroids[i]) ** 2])
